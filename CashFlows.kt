...
//            val tx = TransactionModel(sender=serviceHub.myInfo.legalIdentities.first().toString(),
//                    receiver = otherParty.name.toString(),
//                    transactionAmount = amount.quantity.toDouble())
//            val responseData = session.sendAndReceive(TransactionModel::class.java, tx)
//
//            var resTransferTxModel = TransactionModel()
...
//            }
            ...
            val ftx = subFlow(FinalityFlow(partSignedTx, FINALISING.childProgressTracker()))
            println("PAY AFTER FinalityFlow")
            val cashService = serviceHub.cordaService(ExternalCashService.Service::class.java)
            val tx = TransactionModel(sender=serviceHub.myInfo.legalIdentities.first().toString(),
                    receiver = otherParty.name.toString(),
                    transactionAmount = amount.quantity.toDouble())
            println("BEFORE APPROVE")
            val txResult = cashService.approveTransferTx(tx)
            println("txResult: ".plus(txResult))
...
class AcceptPayment(val otherFlow: FlowSession) : FlowLogic<Unit>() {
    @Suspendable
    override fun call() {
//        val untrustworthyData = otherFlow.receive(TransactionModel::class.java)
//        var valueToSend = TransactionModel()
//        untrustworthyData.unwrap({
//            valueToSend = it
//        })
//
//        val cashService = serviceHub.cordaService(ExternalCashService.Service::class.java)
//        val txResult = cashService.approveTransferTx(valueToSend)
//        otherFlow.send(txResult)
...
//        }
        ...
        println("AFTER PAY berore return")
        val txId = stx.tx.id
